...
- **Apple на устройстве** — системная модель Apple Intelligence. Требует iOS 26 и совместимого
  устройства, доступна не во всех регионах.
- **Своя модель** — открытая модель (Qwen3 0.6B / 1.7B / 4B), которая скачивается с Hugging Face
  при первом вопросе и дальше работает офлайн.

Два последних варианта не требуют ни ключа, ни сети во время игры. Расплата ощутимая: маленькая
модель хуже держит многошаговые рассуждения, не умеет смотреть картинки заданий, и поиск в
интернете для неё отключён. Каталог инструментов, политика доступа, подтверждения и троттлинг у
всех вариантов одни и те же — меняется только то, кто выбирает инструмент.
